...
<body>
    <div class="header">
        <table>
            <tr>
                <td>
                    <img src="{{ public_path('logo/bjm.png') }}" alt="Logo">
                </td>
                <td>
    ...
    <div class=" info-box">
        <p><strong>Filter:</strong>
            @if(isset($filters['start_date']) && $filters['start_date']) Tanggal Mulai: {{
            $filters['start_date'] }}
            @endif
            @if(isset($filters['end_date']) && $filters['end_date']) - Tanggal Selesai: {{
            $filters['end_date'] }}
            @endif
            @if(isset($filters['layanan_id']) && $filters['layanan_id']) - Layanan: {{
            $layanans->find($filters['layanan_id'])->nama ?? 'Semua' }} @endif
        </p>
        <p><strong>Total Data:</strong> {{ count($jawabans) }} Jawaban | <strong>Tanggal Cetak:</strong> {{
            date('d/m/Y
            H:i:s') }}</p>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            if (strtolower($jawaban->jawaban) == 'baik' || strtolower($jawaban->jawaban) == 'puas' ||
            strtolower($jawaban->jawaban) == 'sangat puas') {
            $badgeClass = 'badge-baik';
            } elseif (strtolower($jawaban->jawaban) == 'buruk' || strtolower($jawaban->jawaban) == 'tidak puas' ||
            strtolower($jawaban->jawaban) == 'sangat tidak puas') {
