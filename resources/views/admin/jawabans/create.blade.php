...
@section('title', 'Create Jawaban')
@section('header', 'Create Jawaban')
@section('breadcrumb', 'Add new jawaban')
...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <form action="{{ route('admin.jawabans.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="pertanyaan_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('pertanyaan_id') border-red-500 @enderror">
                        ...
                        <option value="{{ $pertanyaan->id }}">{{ Str::limit($pertanyaan->pertanyaan, 60) }}</option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <textarea name="jawaban" rows="4" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('jawaban') border-red-500 @enderror">{{ old('jawaban') }}</textarea>
