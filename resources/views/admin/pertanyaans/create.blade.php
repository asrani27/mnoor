...
@section('breadcrumb', 'Add new pertanyaan')
...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <form action="{{ route('admin.pertanyaans.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Pertanyaan</label>
                <textarea name="pertanyaan" rows="4" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('pertanyaan') border-red-500 @enderror">{{ old('pertanyaan') }}</textarea>
