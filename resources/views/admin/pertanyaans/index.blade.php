...
@section('breadcrumb', 'Manage pertanyaan')
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200">
    <div class="p-6 border-b border-gray-200">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div>
                <h2 class="text-lg font-semibold text-gray-800">All Pertanyaan</h2>
                <p class="text-sm text-gray-500 mt-1">Manage pertanyaan data</p>
            </div>
            <a href="{{ route('admin.pertanyaans.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700">Add Pertanyaan</a>
        </div>
    </div>

    <div class="p-4 border-b border-gray-200">
        <form action="{{ route('admin.pertanyaans.index') }}" method="GET" class="flex items-center">
    ...
    @if($pertanyaans->hasPages())<div class="px-6 py-4 border-t">{{ $pertanyaans->links() }}</div>@endif
</div>
@endsection
